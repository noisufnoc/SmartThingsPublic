/*
 *
 *  Modified by: Mike Walker (me@noisufnoc.com)
 *  Modified and extended from Kyse's Aeon Minimote Device Handler Template.
 *    which was...
...
metadata {
	definition (name: "noisufnoc's aeon minimote", namespace: "noisufnoc", author: "Mike Walker") {
		capability "Actuator"
		capability "Button"
...
def buttonEvent(button, held) {
	...
	if (held) {
		createEvent(name: "button", value: "{\"button\": $button, \"action\": \"held\"}", data: [buttonNumber: button, action: (held ? "held" : "pushed")], source: "DEVICE", descriptionText: "$device.displayName button $button was held", isStateChange: true)
	} else {
		createEvent(name: "button", value: "{\"button\": $button, \"action\": \"pushed\"}", data: [buttonNumber: button, action: (held ? "held" : "pushed")], source: "DEVICE", descriptionText: "$device.displayName button $button was pushed", isStateChange: true)
	}
}
